function UIsetup() {
  ...
  const col1 = createElement('div', ['col-xs-6']);
  const col2 = createElement('div', ['col-xs-6']);

  const statsCol1 = stats.slice(0, 3);
  const statsCol2 = stats.slice(3);

  console.log("Appending stats to columns");
  appendStatsToColumn(col1, statsCol1);
  appendStatsToColumn(col2, statsCol2);

  row.appendChild(col1);
  row.appendChild(col2);

  const container = document.querySelector("#collapseStatusScreen .panel-body");
  if (container) {
...
function appendStatsToColumn(column, statsArray) {
  statsArray.forEach(stat => {
    const statRow = createElement('div', ['row']);
    const statCol = createElement('div', ['col-xs-4', 'player-stats'], {}, `${stat.label}: <span id="${stat.id}">0</span>`);
    const progressCol = createElement('div', ['col-xs-8']);
...
/**
 * Creates a new DOM element with specified attributes, classes, and inner HTML.
 * @param {string} tagName - The type of element to create (e.g., 'div', 'span').
 * @param {Array<string>} [classNames=[]] - An array of class names to add to the element.
 * @param {Object} [attributes={}] - An object where keys are attribute names and values are attribute values.
 * @param {string} [innerHTML=''] - HTML content to set for the element.
 * @returns {HTMLElement} The created DOM element.
 */
function createElement(tagName, classNames = [], attributes = {}, innerHTML = '') {
  ...
  // Set the inner HTML of the element, if provided
