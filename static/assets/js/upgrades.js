var Upgrades = function() {
	...
	self.gainExcelia = function(monster) {
		var gain = characterBuffs.get('ExceliaMultiplier') * (monster.strength + monster.constitution + monster.dexterity)/15;
	...
	var activateUpgrade = function(upgrade) {
		if (upgrade.id == "timewarp1" || upgrade.id == "timewarp2" || upgrade.id == "timewarp3") {
			...
			characterBuffs.set('ManaPerSecond', characterBuffs.get('ManaPerSecond') + 1);
		}
		else if (upgrade.id == "battleHealing") {
			characterBuffs.set('CastCureInBattle', true);
		}
		else if (upgrade.id == "doubleexcelia1" || upgrade.id == "doubleexcelia2") {
			characterBuffs.set('ExceliaMultiplier', characterBuffs.get('ExceliaMultiplier') * 2);
		}
		else if (upgrade.id == "adeptmage") {
			characterBuffs.set('SpellLevelingMultiplier', characterBuffs.get('SpellLevelingMultiplier') * 2);
		}
		else if (upgrade.id == "blessings1" || upgrade.id == "blessings2" || upgrade.id == "blessings3") {
			characterBuffs.set('ExceliaSavedOnDeath', characterBuffs.get('ExceliaSavedOnDeath') + 10);
		}
		else if (upgrade.id == "autoshoot") {
			characterBuffs.set('CastFireballInBattle', true);
		}
		else if (upgrade.id == "fastresting1" || upgrade.id == "fastresting2" || upgrade.id == "fastresting3") {
			characterBuffs.set('RestingMultiplier', characterBuffs.get('RestingMultiplier') * 2);
		}
		else if (upgrade.id == "musclememory1" || upgrade.id == "musclememory2") {
			characterBuffs.set('DeathPenaltyReduction', characterBuffs.get('DeathPenaltyReduction') + 1);
		}
		else if (upgrade.id == "barriercast") {
			characterBuffs.set('AutoBarrierCast', true);
		}
		else if (upgrade.id == "fasterleveling1" || upgrade.id == "fasterleveling2" || upgrade.id == "fasterleveling3") {
			characterBuffs.set('LevelingSpeedMultiplier', characterBuffs.get('LevelingSpeedMultiplier') * 2);
		}
		else if (upgrade.id == "fasterexploration1" || upgrade.id == "fasterexploration2" || upgrade.id == "fasterexploration3") {
			characterBuffs.set('ExplorationSpeedMultiplier', characterBuffs.get('ExplorationSpeedMultiplier') * 2);
		}
	};
};
